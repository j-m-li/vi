/*
 * Minimal MIPS-I assembler (C90)
 * Features:
 * - Supports MIPS-I instructions (R/I/J types)
 * - Handles labels, .text/.data/.word/.asciiz/.align/.space/.globl
 * - Supports pseudo-instructions (move, li, la, b)
 * - Simple symbol and relocation tables for (basic) linker support
 * - Error handling for common issues
 *
 * Usage:
 *   gcc -std=c90 -o mips_assembler mips_assembler.c
 *   ./mips_assembler input.asm output.bin
 */
...
int main(int argc, char *argv[]) {
    ...
    // Output binary and symbol/reloc tables
...
void parse_line(const char *orig_line, int pass) {
    ...
    if (clean_line[0] == '.') {
        ...
        if (strcmp(mnemonic, ".data") == 0) {
            ...
            // Mark the next label as global
            is_global = 1;
            // In pass 1, the next encountered label will be marked global
            // (Handled by add_label)
        } else {
    ...
    // Parse instructions and pseudo-instructions
    if (sscanf(clean_line, "%s %[^\n]", mnemonic, args) == 2) {
        // Pseudo-instructions
        if (strcmp(mnemonic, "move") == 0) {
            // move $d, $s -> add $d, $s, $zero
            int rd, rs;
            sscanf(args, "$%d, $%d", &rd, &rs);
            char buf[32];
            sprintf(buf, "$%d, $%d, $0", rd, rs);
            if (pass == 2 && in_text_segment)
                memory[pc/4] = encode_instruction("add", buf, pass, pc);
            if (in_text_segment)
                pc += 4;
            return;
        }
        if (strcmp(mnemonic, "li") == 0) {
            // li $d, imm -> addi $d, $zero, imm (if fits) or lui/ori
            int rd, imm;
            sscanf(args, "$%d, %d", &rd, &imm);
            if (imm >= -32768 && imm <= 65535) {
                char buf[32];
                sprintf(buf, "$%d, $0, %d", rd, imm);
                if (pass == 2 && in_text_segment)
                ...
                sprintf(buf1, "$%d, %d", rd, upper);
                sprintf(buf2, "$%d, $%d, %d", rd, rd, lower);
                if (pass == 2 && in_text_segment) {
                    memory[pc/4] = encode_instruction("lui", buf1, pass, pc);
                    memory[pc/4 + 1] = encode_instruction("ori", buf2, pass, pc + 4);
                }
                if (in_text_segment)
                    pc += 8;
            }
        ...
        if (strcmp(mnemonic, "b") == 0) {
            ...
            if (pass == 2 && addr != -1)
                offset = (addr - (pc + 4)) / 4;
            char buf[32];
            sprintf(buf, "$0, $0, %d", offset);
            if (pass == 2 && in_text_segment)
                memory[pc/4] = encode_instruction("beq", buf, pass, pc);
            if (in_text_segment)
                pc += 4;
            return;
        }
...
unsigned int encode_instruction(const char *mnemonic, const char *args, int pass, int cur_pc) {
    int rs = 0, rt = 0, rd = 0, shamt = 0, imm = 0, address = 0;
    char label[MAX_LABEL_LENGTH];
    // R-type
    if (strcmp(mnemonic, "add") == 0) {
        sscanf(args, "$%d, $%d, $%d", &rd, &rs, &rt);
        return (rs << 21) | (rt << 16) | (rd << 11) | (0 << 6) | 0x20;
    } else if (strcmp(mnemonic, "sub") == 0) {
        sscanf(args, "$%d, $%d, $%d", &rd, &rs, &rt);
        return (rs << 21) | (rt << 16) | (rd << 11) | (0 << 6) | 0x22;
    } else if (strcmp(mnemonic, "and") == 0) {
        sscanf(args, "$%d, $%d, $%d", &rd, &rs, &rt);
        return (rs << 21) | (rt << 16) | (rd << 11) | (0 << 6) | 0x24;
    } else if (strcmp(mnemonic, "or") == 0) {
        sscanf(args, "$%d, $%d, $%d", &rd, &rs, &rt);
        return (rs << 21) | (rt << 16) | (rd << 11) | (0 << 6) | 0x25;
    } else if (strcmp(mnemonic, "xor") == 0) {
        sscanf(args, "$%d, $%d, $%d", &rd, &rs, &rt);
        return (rs << 21) | (rt << 16) | (rd << 11) | (0 << 6) | 0x26;
    } else if (strcmp(mnemonic, "nor") == 0) {
        sscanf(args, "$%d, $%d, $%d", &rd, &rs, &rt);
        return (rs << 21) | (rt << 16) | (rd << 11) | (0 << 6) | 0x27;
    } else if (strcmp(mnemonic, "slt") == 0) {
        sscanf(args, "$%d, $%d, $%d", &rd, &rs, &rt);
        return (rs << 21) | (rt << 16) | (rd << 11) | (0 << 6) | 0x2A;
    } else if (strcmp(mnemonic, "sll") == 0) {
        sscanf(args, "$%d, $%d, %d", &rd, &rt, &shamt);
        return (0 << 21) | (rt << 16) | (rd << 11) | (shamt << 6) | 0x00;
    } else if (strcmp(mnemonic, "srl") == 0) {
        sscanf(args, "$%d, $%d, %d", &rd, &rt, &shamt);
        return (0 << 21) | (rt << 16) | (rd << 11) | (shamt << 6) | 0x02;
    ...
    // I-type
        ...
        sscanf(args, "$%d, %d", &rt, &imm);
        return (0x0F << 26) | (0 << 21) | (rt << 16) | (imm & 0xFFFF);
    } else if (strcmp(mnemonic, "lw") == 0) {
        sscanf(args, "$%d, %d($%d)", &rt, &imm, &rs);
    ...
    // J-type
        ...
        int op = (strcmp(mnemonic, "j") == 0) ? 0x02 : 0x03;
        if (sscanf(args, "%s", label) == 1) {
            address = get_label_address(label);
            if (address == -1) {
                if (pass == 2) add_reloc(label, cur_pc);
                address = 0;
            }
        } else {
            sscanf(args, "%d", &address);
        }
        return (op << 26) | ((address>>2) & 0x3FFFFFF);
    }
    // NOP
